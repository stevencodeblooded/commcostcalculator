/**
 * Chart visualization logic for the Contractor Communication Cost Calculator
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // These settings can be used to further customize the chart if needed
  const chartConfig = {
    // Color palette for the chart (used in costBreakdown)
    colors: {
      paymentDelay: "#ef4444",
      dispute: "#f97316",
      timeWaste: "#eab308",
      lostOpportunity: "#8b5cf6",
      stress: "#ec4899",
    },
  ...
  /**
   * Creates and animates the cost breakdown chart using Chart.js
   * @param {Array} costBreakdown - Array of cost breakdown items with values and colors
   * @param {string} chartId - The ID of the chart canvas element
   */
  function createAnimatedChart(costBreakdown, chartId = "costBreakdownChart") {
    ...
    chartCanvas.addEventListener("click", function (event) {
      const activePoints = window.costChart.getElementsAtEventForMode(
        event,
        "nearest",
        { intersect: true },
        false
      ...
      if (activePoints.length > 0) {
        ...
        const dataset = window.costChart.data.datasets[0];
